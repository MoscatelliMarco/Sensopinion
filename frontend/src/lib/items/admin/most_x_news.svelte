<script>
    ...
    $: if (parameters) {
        news_article_show = news_articles.filter((item) => {
            if (parameters['category'] == 'politics' || parameters['category'] == 'economy' || parameters['category'] == 'environment') {
                if (Object.keys(item['categories']).includes(parameters['category'].charAt(0).toUpperCase() + parameters['category'].slice(1))) {
                    return true
                }
            } else {
                for (let category in item['categories']) {
                    if (item['categories'][category].includes(parameters['category'] ? capitalizeFirstLetter(parameters['category'].replace("_politics", "").replace("_environment", "").replace("_economy", "").replaceAll("_", " ")) : parameters['category'])) {
                        return true;
                    }
                }
            }
        ...
        news_article_show = news_article_show.sort((a, b) => {
            let factor1;
            let factor2;
...
<main class="w-full h-full pt-14 pb-20 px-28 flex flex-col justify-between gap-4">
    ...
    <div class="flex justify-between gap-8">
        <div class="flex flex-col justify-center">
            ...
            {/if}
            <p class="font-light text-center w-full mt-2 text-grey-2">*{parameters['factor_name_progress']} of the news highlighted</p>
        </div>
        <div class="flex flex-col justify-center gap-1.5 max-w-md text-grey-1">
            <div class="flex gap-2">
                <div class="flex flex-col justify-center">
                    <h2 class="text-3xl font-medium">
                        <svg class="w-9 inline-block" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1024 1024" version="1.1">
                        ...
                        {news_article_show[0] ? news_article_show[0]['title'].slice(0, 80).replace(/\s$/, '') : ""}{news_article_show[0] ? news_article_show[0]['title'].length > 80 ? "..." : "" : ""}</h2>
                </div>
            </div>
            <div class="overflow-hidden w-full h-52">
                <img src="{news_article_show[0] ? `/api/admin/${import.meta.env.VITE_ADMIN_CORS_CODE}/bypass_cors/` : ""}{news_article_show[0] ? encodeURIComponent(news_article_show[0]['image']) : ""}" class="object-cover w-full h-full rounded-lg card-images" alt="placeholder alt"/>
            </div>
            <div class="mt-0.5 font-light text-lg">
    ...
    <div class="flex flex-col gap-4 mb-4 justify-center text-grey-1 mt-6">
        ...
        <div class="flex w-full gap-4">
            <div class="flex gap-6">
                ...
                <div class="flex flex-col justify-center">
                    {#if news_article_show[1]}
                        <NewsChartCustom size={120} thickness={10} font_size={16} font_weight={600} value={parameters['factor'] != 'polarity' && parameters['factor'] != 'subjectivity' ? Math.round((parameters['1_minus_x'] == 'yes' ? 1 - news_article_show[1]['emotions'][parameters['factor']] : news_article_show[1]['emotions'][parameters['factor']]) * 1000) / 10 : Math.round($globalStore.stretchFunction(parameters['1_minus_x'] == 'yes' ? 1 - news_article_show[1]['sentiment'][parameters['factor']] : news_article_show[1]['sentiment'][parameters['factor']]) * 1000) / 10}/>
                    {/if}
                </div>
            </div>
            <div class="overflow-hidden h-32 w-48 shrink-0 flex flex-col justify-center">
                <img src="{news_article_show[1] ? `/api/admin/${import.meta.env.VITE_ADMIN_CORS_CODE}/bypass_cors/` : ""}{news_article_show[1] ? encodeURIComponent(news_article_show[1]['image']) : ""}" class="object-cover w-full h-full rounded-md card-images" alt="placeholder alt"/>
            </div>
            <div class="flex flex-col justify-center gap-1">
                <h3 class="font-bold text-xl">{news_article_show[1] ? news_article_show[1]['title'].slice(0, 100).replace(/\s$/, '') : ""}{news_article_show[1] ? news_article_show[1]['title'].length > 100 ? "..." : "" : ""}</h3>
                <p class="text-base font-light">{news_article_show[1] ? news_article_show[1]['description'].slice(0, 150).replace(/\s$/, '') : ""}{news_article_show[1] ? news_article_show[1]['description'].length > 150 ? "..." : "" : ""}</p>
            </div>
        </div>
        ...
        <div class="flex w-full gap-4">
            <div class="flex gap-6">
                ...
                <div class="flex flex-col justify-center">
                    {#if news_article_show[2]}
                        <NewsChartCustom size={120} thickness={10} font_size={16} font_weight={600} value={parameters['factor'] != 'polarity' && parameters['factor'] != 'subjectivity' ? Math.round((parameters['1_minus_x'] == 'yes' ? 1 - news_article_show[2]['emotions'][parameters['factor']] : news_article_show[2]['emotions'][parameters['factor']]) * 1000) / 10 : Math.round($globalStore.stretchFunction(parameters['1_minus_x'] == 'yes' ? 1 - news_article_show[2]['sentiment'][parameters['factor']] : news_article_show[2]['sentiment'][parameters['factor']]) * 1000) / 10}/>
                    {/if}
                </div>
            </div>
            <div class="overflow-hidden h-32 w-48 shrink-0 flex flex-col justify-center">
                <img src="{news_article_show[2] ? `/api/admin/${import.meta.env.VITE_ADMIN_CORS_CODE}/bypass_cors/` : ""}{news_article_show[2] ? encodeURIComponent(news_article_show[2]['image']) : ""}" class="object-cover w-full h-full rounded-md card-images" alt="placeholder alt"/>
            </div>
            <div class="flex flex-col justify-center gap-1">
                <h3 class="font-bold text-xl">{news_article_show[2] ? news_article_show[2]['title'].slice(0, 100).replace(/\s$/, '') : ""}{news_article_show[2] ? news_article_show[2]['title'].length > 100 ? "..." : "" : ""}</h3>
                <p class="text-base font-light">{news_article_show[2] ? news_article_show[2]['description'].slice(0, 150).replace(/\s$/, '') : ""}{news_article_show[2] ? news_article_show[2]['description'].length > 150 ? "..." : "" : ""}</p>
            </div>
        </div>
