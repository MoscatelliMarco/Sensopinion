    ...
    <div class="flex">
        <div class="flex items-start">
            <div class="flex flex-col gap-4 p-3">
                <button on:click={() => {filterActive ? setTimeout(() => {if (!filterActive) {filterShow = false}}, 300) : filterShow = true; filterActive = !filterActive; sortActive = false; sortShow = false; if(!filterActive && !sortActive) {firstOpen = true} else {setTimeout(() => {firstOpen = false}, 25)}}}>
                ...
                <button on:click={() => {sortActive ? setTimeout(() => {if (!sortActive) {sortShow = false}}, 300) : sortShow = true; sortActive = !sortActive; filterActive = false; filterShow = false; if(!filterActive && !sortActive) {firstOpen = true;} else {setTimeout(() => {firstOpen = false}, 25)}}}>
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-6 stroke-black" viewBox="0 0 24 24" fill="none">
