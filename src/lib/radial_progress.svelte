<script>
    ...
    onMount(() => {
        // Right value for stroke offsets
        let circle = document.querySelector(`#${name}`)
        circle.style['stroke-dasharray'] = 2 * Math.PI * (size / 2 - thickness / 2) + "px"
        circle.style['stroke-dashoffset'] = 2 * Math.PI * (size / 2 - thickness / 2) + "px"
...
" class="relative">
    ...
    <svg xlmns="http://www.w3.org/2000/svg" version="1.1" width="{size}" height="{size}">
        ...
        <circle style="
            ...
            animation: {name} 0.5s ease-in forwards;
...
<style>
    circle {
        ...
        /* TODO put exactly the circumference here */
